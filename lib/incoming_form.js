...
function IncomingForm(opts) {
  ...
  this.isAutoRename = opts.isAutoRename || true;
...
IncomingForm.prototype._uploadPath = function(filename) {
  //if isAutoRename is false
  //return filename without modification
  var _path = '';
  if(this.isAutoRename === false) {
	  var execRes = /(?:^.*\/)?(.*)$/.exec(filename);
	  var fileName = execRes && execRes[1] || filename;
	  _path = path.join(this.uploadDir, fileName);
  } else {
	  var name = '';
	  for (var i = 0; i < 32; i++) {
	  ...
	  _path = path.join(this.uploadDir, name);
  }
  return (_path);
};
