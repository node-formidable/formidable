...
const PORT = 13539;
const uploadsDir = './test-node/uploads';
...
`;
    fetch(String(new URL(`http:localhost:${chosenPort}/`)), {
      ...
      deepEqual(fs.readdirSync(uploadsDir).includes('x'), true);
      deepEqual(fs.readdirSync(`${uploadsDir}/x`), ['y']);
      deepEqual(fs.readdirSync(`${uploadsDir}/x/y`), ['z.txt']);
